...
#     NOTE: this assumes non-overlapping chunks (each chunk is a fresh,
#     fully-drained audio buffer, never re-transcribed audio) — which is
#     how interview_page.py's TranscriptionWorker feeds it. Because of
#     that, the only duplicate we actually need to guard against is the
#     exact same segment being flushed twice in a row; we do NOT want to
#     drop a new segment just because its text happens to already appear
#     somewhere earlier in the transcript (a previous version did this via
#     `normalized_new in normalized_existing`, which silently discarded
#     any new word/phrase that was a substring of anything already said —
#     e.g. a fresh "ready" segment being dropped because "already" was
#     said earlier). That was eating the large majority of real chunks.
#     """
#     cleaned_new = (new_text or "").strip()
#     if not cleaned_new:
#         return existing_text.strip()
